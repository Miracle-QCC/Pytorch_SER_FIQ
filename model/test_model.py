    ...
    def euclidean_dist(self, x):
        ...
        for i in range(m):
            for j in range(i+1,m):
                out += torch.mean(torch.sqrt((x[:,i,:] - x[:,j,:]).pow(2)), dim=-1)
...
if __name__ == '__main__':
    ...
    # model(input)
    # checkpoint = {"model":SER_FIQ(m=10),
    #               'model_state_dict':model.state_dict(),
    #          'optimize_state_dict':optimizer.state_dict(),
    #          'epoch':0}
    # torch.save(checkpoint,"xxx.pth")
    # model(input)
    # input_names = ['input']
    # output_names = ['output']
    # torch.onnx.export(model, input, 'SER_FIQ.onnx', input_names=input_names, output_names=output_names, verbose='True')
